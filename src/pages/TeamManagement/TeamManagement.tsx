...
export default function TeamManagementPage() {
  ...
  const allSkills = Array.from(
    new Set(
      requests.flatMap((r) => r.user?.skills?.map((s: any) => s.name) || []),
    ),
  );
  const filteredRequests = requests.filter((r) => {
    const matchesSearch = r.user?.name
      ?.toLowerCase()
      .includes(searchQuery.toLowerCase());
    const matchesSkill =
      !skillFilter || r.user?.skills?.some((s: any) => s.name === skillFilter);
          ...
          {roles.length === 0 ? (
            ...
            >
              ...
              <p className="mt-4 text-sm font-bold text-slate-600">
                Belum ada role di tim ini
              </p>
              <p className="text-xs text-slate-400">
                Tambahkan role untuk mulai merekrut member.
              </p>
              <Button
                size="sm"
              ...
              <AnimatePresence mode="popLayout">
                {roles.map((role) => (
                  <RoleCard
                    ...
                    onRemoveMember={(memberId: string | number) =>
                      handleRemoveMember(id as string, memberId)
                    }
                    onViewMember={(m: any) => {
                      setSelectedMember(m);
                      setDrawerOpen(true);
                  ...
                  <JoinRequestCard
                    ...
                    request={{
                      ...
                      skills: req.user.skills || [
                        ...
                        "Next.js",
                        "TypeScript",
                      ], // Dummy skills jika API belum kirim
                    }}
