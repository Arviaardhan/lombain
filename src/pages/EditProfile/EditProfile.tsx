...
import { ArrowLeft, Camera, X, Plus, Save, Github, Linkedin, Globe, User, GraduationCap, FileText, Wrench, Link2, Loader2 } from "lucide-react";
...
const CATEGORIES = [
  ...
  { value: "3", label: "Creative & Design" },
  { value: "4", label: "Science & Research" },
];
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-12 pb-24 bg-slate-50/30">
      <div className="flex items-center justify-between mb-10">
        ...
        <Button onClick={handleSave} disabled={saving} className="bg-[#5A8D39] hover:bg-[#4a752f] rounded-xl px-6 gap-2 shadow-lg shadow-green-100">
      ...
      <div className="space-y-6">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      className="rounded-xl border-slate-100 focus:border-[#5A8D39] h-12" 
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Major</Label>
                    <Input 
        ...
        <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/40">
          <CardContent className="p-8">
            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 block">About Me</Label>
            <Textarea
              value={formData.bio}
              onChange={(e) => setFormData({...formData, bio: e.target.value})}
              rows={4}
              placeholder="Describe your passion and achievements..."
              className="rounded-2xl border-slate-100 focus:border-[#5A8D39] text-sm leading-relaxed resize-none"
            />
          </CardContent>
        </Card>
        ...
        <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/40">
          <CardContent className="p-8">
            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 block text-center">Skills & Expertise</Label>
            
            <div className="flex flex-wrap justify-center gap-2 mb-8">
              {formData.skills.map((skill) => (
                <Badge key={skill} className="bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border-none px-4 py-2 rounded-full text-xs font-bold gap-2">
                  {skill}
